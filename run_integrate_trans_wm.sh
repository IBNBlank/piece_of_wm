#!/usr/bin/env bash
###############################################################################
# Run Trans-WM pretraining, formal training, and final online evaluation.
#
# Usage:
#   ./run_integrate_trans_wm.sh
#   DEVICE=cuda PRETRAIN_EPOCHS=100 TRAIN_ROLLOUTS=500 ./run_integrate_trans_wm.sh
#
# Shared:
...
if [ ! -f "${trained_checkpoint}" ]; then
	echo "[run_integrate_trans_wm] error: training did not produce ${trained_checkpoint}" >&2
	exit 1
fi

echo "[run_integrate_trans_wm] stage=eval checkpoint=${trained_checkpoint}"
env \
	MODEL="trans_wm" \
	TRANS_WM_CHECKPOINT="${trained_checkpoint}" \
