"""Train Dreamer-like from a directory of sequence-preserving rollout files."""
...
LOGGER = logging.getLogger("piece_of_wm.dreamer_like")
...
_run_pretraining = partial(
    ...
    description="Pretraining Dreamer-like",
    logger=LOGGER,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/dreamer_like"))
...
def _load_rollout(path: Path) -> EpisodeBatch:
    ...
    if batch.images is None:
        raise ValueError(f"Rollout {path} has no images; Dreamer-like requires image data.")
