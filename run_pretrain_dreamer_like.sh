#!/usr/bin/env bash
###############################################################################
# Pretrain Dreamer-like from offline replay data.
#
# Usage:
#   ./run_pretrain_dreamer_like.sh
#   DEVICE=cuda EPOCHS=10 ./run_pretrain_dreamer_like.sh
###############################################################################
set -Eeuo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-runs/dreamer_like_pretrain}"
...
echo "[run_pretrain_dreamer_like] data_dir=${DATA_DIR} output_dir=${OUTPUT_DIR}"
echo "[run_pretrain_dreamer_like] epochs=${EPOCHS} batch_size=${BATCH_SIZE} planning_horizon=${PLANNING_HORIZON} device=${DEVICE:-auto}"
