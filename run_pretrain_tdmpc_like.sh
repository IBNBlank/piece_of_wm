#!/usr/bin/env bash
###############################################################################
# Pretrain TD-MPC-like from offline replay data.
#
# Usage:
#   ./run_pretrain_tdmpc_like.sh
#   DEVICE=cuda EPOCHS=10 ./run_pretrain_tdmpc_like.sh
###############################################################################
set -Eeuo pipefail
...
if [ ! -x "${PYTHON}" ]; then
	echo "[run_pretrain_tdmpc_like] error: Python executable not found: ${PYTHON}" >&2
	exit 1
fi

DATA_DIR="${DATA_DIR:-dataset/fetch-pick-and-place-random}"
OUTPUT_DIR="${OUTPUT_DIR:-runs/tdmpc_like_pretrain}"
...
echo "[run_pretrain_tdmpc_like] data_dir=${DATA_DIR} output_dir=${OUTPUT_DIR}"
echo "[run_pretrain_tdmpc_like] epochs=${EPOCHS} batch_size=${BATCH_SIZE} planning_horizon=${PLANNING_HORIZON} device=${DEVICE:-auto}"
