#!/usr/bin/env bash
###############################################################################
# Run TD-MPC-like pretraining, formal training, and final online evaluation.
#
# Usage:
#   ./run_integrate_tdmpc_like.sh
#   DEVICE=cuda PRETRAIN_EPOCHS=100 TRAIN_ROLLOUTS=500 ./run_integrate_tdmpc_like.sh
#
# Shared:
...
#                          (default: runs/tdmpc_like_pretrain)
...
PRETRAIN_OUTPUT_DIR="${PRETRAIN_OUTPUT_DIR:-runs/tdmpc_like_pretrain}"
...
TRAIN_OUTPUT_DIR="${TRAIN_OUTPUT_DIR:-runs/tdmpc_like}"
...
if [ "${PRETRAIN_OUTPUT_DIR}" = "${TRAIN_OUTPUT_DIR}" ]; then
	echo "[run_integrate_tdmpc_like] error: pretraining and training output directories must differ" >&2
	exit 1
fi
echo "[run_integrate_tdmpc_like] stage=pretrain output_dir=${PRETRAIN_OUTPUT_DIR}"
env \
	DATA_DIR="${DATA_DIR}" \
	...
	"${SCRIPT_DIR}/run_pretrain_tdmpc_like.sh"

pretrained_checkpoint="${PRETRAIN_OUTPUT_DIR}/checkpoint_best.pt"
if [ ! -f "${pretrained_checkpoint}" ]; then
	echo "[run_integrate_tdmpc_like] error: pretraining did not produce ${pretrained_checkpoint}" >&2
	exit 1
fi
...
echo "[run_integrate_tdmpc_like] stage=train output_dir=${TRAIN_OUTPUT_DIR}"
env \
	DATA_DIR="${DATA_DIR}" \
	...
	"${SCRIPT_DIR}/run_train_tdmpc_like.sh"

trained_checkpoint="${TRAIN_OUTPUT_DIR}/checkpoint_best.pt"
if [ ! -f "${trained_checkpoint}" ]; then
	echo "[run_integrate_tdmpc_like] error: training did not produce ${trained_checkpoint}" >&2
	exit 1
fi

echo "[run_integrate_tdmpc_like] stage=eval checkpoint=${trained_checkpoint}"
env \
	MODEL="tdmpc_like" \
	TDMPC_LIKE_CHECKPOINT="${trained_checkpoint}" \
