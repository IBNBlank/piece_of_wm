...
from tdmpc_like.config import ACTION_HISTORY_LEN, OBS_HISTORY_LEN
from tdmpc_like.history import append_history, history_windows, previous_history_windows
from tdmpc_like.model import WorldModel
...
def transition_batch_from_indices(
    ...
    source = batch.images if batch.images is not None else batch.obs
    if source.ndim != 5:
        raise ValueError("TD-MPC-like training requires image observations.")
